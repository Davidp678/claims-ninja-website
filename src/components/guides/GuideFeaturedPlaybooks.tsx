...
export function GuideFeaturedPlaybooks({
  ...
  return (
    <>
      {primary ? <GuideFeaturedPlaybookBanner guide={primary} locale={locale} /> : null}
      {secondary.length > 0 ? (
          ...
          <ul className="mt-6 grid gap-4 sm:grid-cols-2">
            {secondary.map((guide) => (
              <li key={`${guide.category}-${guide.slug}`}>
                <GuideFeaturedCompactCard guide={guide} locale={locale} />
              </li>
            ))}
          </ul>
        </Section>
...
export function GuideFeaturedPlaybookBanner({
  guide,
  locale = "en",
}: {
  guide: Guide;
...
function GuideFeaturedCompactCard({
  guide,
  locale = "en",
}: {
  guide: Guide;
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 line-clamp-2 flex-1 text-sm text-zinc-400">{summary.excerpt}</p>

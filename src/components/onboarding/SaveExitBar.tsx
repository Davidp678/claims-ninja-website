...
export function SaveExitBar({
  ...
  const statusLabel =
    saveState === "error" ? "Save failed — retry" : hint;

  return (
    <div
      className={cn(
        "onboarding-save-exit border-t border-white/10 bg-brand-elevated/90",
        className,
      )}
    >
      <div className="flex w-full flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        <div className="flex min-w-0 flex-wrap items-center gap-3">
          <button
            type="button"
            onClick={onSaveExit}
            className="rounded-lg border border-white/25 bg-transparent px-4 py-2.5 text-sm font-medium text-white transition hover:border-white/45 hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-red-light"
          >
            Save & exit
          </button>
          <p
